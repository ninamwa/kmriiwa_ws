...
from kmr_communication.msg import KmpStatusdata
...
class Test(Node):
    def __init__(self):
        ...
        sub_status = self.create_subscription(LaserScan, 'scan', self.status_callback, qos_profile_sensor_data
    ...
    def status_callback(self,data):
        if (data.warning_field_clear != self.warning_field_clear and self.get_clock().now().toSec()-self.last_update_time > 5.0):
            ...
            self.last_update_time = self.get_clock().now().toSec()
